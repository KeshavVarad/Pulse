...
const TutorialAdmin = () => {
    ...
    useEffect(() => {
        const fetchTutorials = async () => {
            ...
            const response = await fetch(`${process.env.REACT_APP_API_HOST}/api/user_guides/${"admin"}`);
            if (response.ok) {
    ...
    return (
        <Box sx={{
            ...
            <Button component={Link} to={`/user_guides/`} sx={{
                textTransform: 'none',
                '&:hover': {
                    ...
                    <Grid item xs={12} sm={6} md={4} key={index}>
                        <Card>
                            <CardActionArea component={Link} to={`/user_guides/admin/${tutorial}/`}>
                                <CardContent>
                                    <Typography variant="h5">{formatString(tutorial.toString())}</Typography>
